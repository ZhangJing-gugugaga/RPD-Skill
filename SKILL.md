...
Follow the user's first-message language throughout. 从首条消息检测语言，全程跟随。
中文 → 中文；English → English；不确定 → 默认中文并询问。
...
5. **Don't overwrite without backup / 不覆盖未备份文件**：写状态文件前先 `state-guard.py --action backup`。
...
- **层1** `code-map.router.json`：常驻 context，≤3k token **且** ≤全码库 15%。
- **层2** `code-map.json`：完整符号表，按 `function:path:name` 查单条（≤300 token），**从不整份喂入**。
...
所有脚本位于 Skill 目录，降级顺序：Skill 目录 → 项目目录 → 手动操作。
**Core principle**: script failure NEVER terminates the Skill. 脚本失败绝不终止流程，按降级表手动继续。
...
| flow | action |
|------|--------|
| scene_exploration | 场景探索（小白生活化引导） |
...
快捷命令：`/rpd new` `/rpd take` `/rpd cont` `/rpd status` `/rpd scan`。

### Flow A: New Project / 新项目
A-1 初始化目录 → A0 问题校验（6 项）→ A1 三视角诊断（用户/商业/技术/安全，每轮≤3 问；小白安全视角自动判）→ A2 概念版 PRD（≤200 字）→ A3 范围冻结确认 → A4 落地版 PRD（模板见 `references/prd-template.md`，含 AI 增强七~十五）→ `prd-validator.py --ai-mode` 校验 → A5 生成状态文件（先 `state-guard.py backup` 再 `state-validator.py`）。
...
### v2 冷启动（新会话接手 v2 项目）
`python scripts/rpd-cold-start.py <root>`：
① 读 active-context ② 红线机检 ③ 声明鲜度 + 显式告警 ④ 读 router ⑤ 定位 ⑥ 动手（verified 前置）⑦ 收尾（更新 active-context + 增量落盘 + decisions）。
详见 `docs/rpd-v2-usage.md`。
